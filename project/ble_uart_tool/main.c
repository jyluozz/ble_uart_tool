...
#define UART_BAUDRATE_ADDR_BASE 0x50000 //存放波特率的地址
#define UART_MODE_ADDR_BASE 0x60000//存放主从模式的地址
...
volatile uint8_t g_ucServer_Master_Mode = 0;
...
static const uint8_t bat_str[] = {0xA5, 0x5A};
static const uint32_t bat_val[] = {1200, 2400, 4800, 9600, 14400, 19200, 38400, 57600, 76800, 115200, 230400, 250000, 460800, 750000, 921600, 100000, 200000};
...
static void mode_switch_key_init(void);
...
static bool g_ucServer_Bat_val = true;
...
static void _LOG_UART(const char *fmt, ...);
...
static void ls_single_role_timer_cb(void *param)
{
    if (g_ucServer_Master_Mode) // server mode
    {
...
static void ls_uart_init(void)
{
    ...
    uint8_t led = 0;
    hal_flash_fast_read(UART_BAUDRATE_ADDR_BASE, (uint8_t *)&bat_value, 1);
    if (bat_value == 0xff)
    {
        bat_value = 9;
    }
    pinmux_uart1_init(PB12, PB13);
    io_pull_write(PB13, IO_PULL_UP);
    UART_Config.UARTX = UART1;
    UART_Config.Init.MSBEN = 0;
    UART_Config.Init.BaudRate = UART_BUADRATE_ENUM_GEN(bat_val[bat_value]);
    ...
    if (g_ucServer_Master_Mode)
    {
        _LOG_UART("******    Current uart mode is server!*\r\n******\r\n");
        led = PA01;
    }
    else
    {
        _LOG_UART("******    Current uart mode is master!\r\n******\r\n");
        led = PA00;
    }
    io_cfg_output(led);
    io_cfg_opendrain(led);
    io_clr_pin(led);

    _LOG_UART("******    Current BaudRate:%d\r\n******\r\n", bat_val[bat_value]);
}
static void _LOG_UART(const char *fmt, ...)
{
#ifdef OPEN_LOG
    ...
    HAL_UART_Transmit(&UART_Config, (uint8_t *)buf, len, 1000);
#endif
}
...
static void ls_uart_server_write_req_ind(uint8_t att_idx, uint8_t con_idx, uint16_t length, uint8_t const *value)
{
    ...
    if (att_idx == UART_SVC_IDX_RX_VAL)
    {
        ...
        if (uart_server_tx_buf[0] == bat_str[0] && uart_server_tx_buf[1] == bat_str[1])
        {
            bat_flag = true;
            for (uint8_t i = 0; i < 4; i++)
            {
                bat_value |= (uart_server_tx_buf[i + 2] & 0xff) << (8 * i);
            }
        }

        if (bat_flag)
        {
            for (uint8_t i = 0; i < (sizeof(bat_val) / 4); i++)
            {
                if (bat_value == bat_val[i])
                {
                    uint32_t cpu_stat = enter_critical();
                    hal_flash_sector_erase(UART_BAUDRATE_ADDR_BASE);
                    hal_flash_page_program(UART_BAUDRATE_ADDR_BASE, (uint8_t *)&i, 1);
                    exit_critical(cpu_stat);
                    _LOG_UART("******    Change the baud rate to %ld\r\n******\r\n", bat_value);
                    platform_reset(1);
                }
            }
        }
...
static void ls_uart_client_recv_ntf_ind(uint8_t handle, uint8_t con_idx, uint16_t length, uint8_t const *value)
{
    ...
    memcpy((void *)&uart_client_tx_buf[0], value, length);
    uint32_t cpu_stat = enter_critical();
    if (!uart_tx_busy)
    {
        uart_tx_busy = true;
        ...
        _LOG_UART("\r\n[client log]:");
        HAL_UART_Transmit_IT(&UART_Config, &uart_client_tx_buf[0], length);
    }
    exit_critical(cpu_stat);
}
static void ls_uart_client_send_write_req(void)
{
    bool bat_flag = false;
    static uint32_t bat_value = 0;
    if (con_idx_client != CON_IDX_INVALID_VAL && uart_client_recv_data_length != 0 && uart_client_wr_cmd_done)
    {
        if (uart_client_ble_buf[0] == bat_str[0] && uart_client_ble_buf[1] == bat_str[1])
        {
            bat_flag = true;
            for (uint8_t i = 0; i < 4; i++)
            {
                bat_value |= (uart_client_ble_buf[i + 2] & 0xff) << (8 * i);
            }
        }
        if (bat_flag)
        {
        ...
        else
        {
            ...
            uint16_t tx_len = uart_client_recv_data_length > co_min(UART_CLIENT_MAX_DATA_LEN, UART_SVC_TX_MAX_LEN) ? co_min(UART_CLIENT_MAX_DATA_LEN, UART_SVC_TX_MAX_LEN) : uart_client_recv_data_length;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (g_ucServer_Master_Mode) // server mode
    {
        ...
        {
            uart_server_ble_buf[uart_server_recv_data_length++] = uart_rx_byte;
        }
        else
        {
            LOG_I("uart server rx buffer overflow!");
        }
        HAL_UART_Receive_IT(&UART_Config, &uart_rx_byte, UART_SYNC_BYTE_LEN);
    }
    else // master mode
    {
        if (uart_client_recv_data_length < UART_SVC_BUFFER_SIZE)
        {
            uart_client_ble_buf[uart_client_recv_data_length++] = uart_rx_byte;
        }
        else
        {
            LOG_I("uart client rx buffer overflow!");
        }
        HAL_UART_Receive_IT(&UART_Config, &uart_rx_byte, UART_SYNC_BYTE_LEN);
    }
}
        ...
        if (!disconnected)
        {
            ...
            case CLIENT_WRITE_WITH_RSP_DONE:
                if (evt->client_write_rsp.status == 0)
                {
                    uint8_t bat_value = 0;
                    LOG_I("write success");
                    if (con_idx_client != CON_IDX_INVALID_VAL && g_ucServer_Bat_val && uart_client_wr_cmd_done)
                    {
                        uint8_t server_bat_val[6] = {0xA5, 0X5A};
                        g_ucServer_Bat_val = false;
                        hal_flash_fast_read(UART_BAUDRATE_ADDR_BASE, (uint8_t *)&bat_value, 1);
                        if (bat_value == 0xff)
                            bat_value = 9;
                        for (uint8_t i = 0; i < 4; i++)
                        {
                            server_bat_val[i + 2] = (bat_val[bat_value] >> (8 * i)) & 0xff;
                        }
                        _LOG_UART("******    set clinet the baud rate to %d\r\n******\r\n", bat_val[bat_value]);
                        gatt_manager_client_write_no_rsp(con_idx_client, uart_client_rx_pointer_handle, &server_bat_val[0], 6);
                    }
                }
                else
                {
...
void io_exti_callback(uint8_t pin,exti_edge_t edge)
{
    ...
    {
        if(g_ucServer_Master_Mode)
            g_ucServer_Master_Mode = 0;
        else
            g_ucServer_Master_Mode = 1;
        uint32_t cpu_stat = enter_critical();
        hal_flash_sector_erase(UART_MODE_ADDR_BASE);
        hal_flash_page_program(UART_MODE_ADDR_BASE, (uint8_t *)&g_ucServer_Master_Mode, 1);
        hal_flash_sector_erase(UART_BAUDRATE_ADDR_BASE);
        exit_critical(cpu_stat);
        if(g_ucServer_Master_Mode)_LOG_UART("******    switch the uart mode to server!\r\n******\r\n");
        else _LOG_UART("******    switch the uart mode to master!\r\n******\r\n");
        platform_reset(1);
    }
    break;
    default:
...
int main()
{
    sys_init_app();
    ble_init();
    hal_flash_fast_read(UART_MODE_ADDR_BASE, (uint8_t *)&g_ucServer_Master_Mode, 1);
